...
public interface TempTrackStorageRepository extends JpaRepository<TempTrackStorage, Long> {
    ...
    @Query("select t from TempTrackStorage t where t.id=:id and t.token = :token and t.user =:user and t.isPlayList =:isPlayList")
    Optional<TempTrackStorage> findOne(@Param("id") Long id, @Param("token") String token, @Param("user") User user, @Param("isPlayList") boolean isPlayList);

    @Query("select t from TempTrackStorage t where  t.user =:user and t.isPlayList =:isPlayList and t.token in (:tokens) and t.id in (:ids)")
    List<TempTrackStorage> findByUid(@Param("user") User user, @Param("tokens") List<String> tokens,
        @Param("ids") List<Long> ids,  @Param("isPlayList") boolean isPlayList);
